...
#define PLL_SYS_KHZ (133 * 1000)  //133 MHz
...
int main() {
    ...
    DEBUG_PRINT("IO initialized\n");

    static DacX578 dac(i2c1, DAC_I2C_ADDRESS, 12);
    dac.begin();
    DEBUG_PRINT("DAC initialized\n");
    // printf("Setting up pico2...\n");

    static PICO_ADS1115 adc;
    adc.beginADSX(PICO_ADS1115::ADSX_ADDRESS_GND, i2c0, 400 * 1000);
    DEBUG_PRINT("ADC initialized\n");

    static PCAL6416A pcagpio(i2c1, 0x21);

    DEBUG_PRINT("GPIO Expander initialized\n");
    ...
    DEBUG_PRINT("MEMS Switches initialized\n");
    ...
    DEBUG_PRINT("MEMS Switch names initialized\n");

    static MEMSRouter router(named_switches, std::size(named_switches));

    DEBUG_PRINT("MEMS Router initialized\n");
    ...
    DEBUG_PRINT("Lightpath routes defined\n");
